      ...
      <div className="flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-surface border border-border shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border bg-[#1a1a19] p-4">
          <div className="flex items-center gap-3">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-accent opacity-80">
              <path d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M14 2V8H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <div>
              <h2 className="text-lg font-semibold text-text-primary leading-tight">{document.filename}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                <p className="text-xs text-text-muted">
                  {(document.fileSize / 1024 / 1024).toFixed(2)} MB • {document.chunkCount} indexed sections
                </p>
        ...
        <div className="flex h-full min-h-0">
          ...
          <div className="w-48 border-r border-border bg-[#151514] p-3 flex flex-col gap-1">
            <button
              ...
              className={cn(
                ...
                activeTab === "summary" ? "bg-accent text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
              )}
            >
              AI Summary
            </button>
            <button
              ...
              className={cn(
                "px-3 py-2 text-sm text-left rounded-md transition-colors font-medium",
                activeTab === "search" ? "bg-accent text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
              )}
            >
              Semantic Search
            </button>
            {isOcr && (
              <button
                ...
                className={cn(
                  "px-3 py-2 text-sm text-left rounded-md transition-colors font-medium",
                  activeTab === "ocr" ? "bg-accent text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
                )}
              >
                OCR Details
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-surface">
            {activeTab === "summary" && (
              <div className="max-w-3xl flex flex-col h-full">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-serif font-semibold text-text-primary">Document Summary</h3>
                ...
                {!summary && !isGeneratingSummary && !summaryError && (
                  <div className="flex-1 flex flex-col items-center justify-center text-center opacity-60">
                    ...
                    <p className="text-lg font-medium text-text-primary">No summary generated yet</p>
                    <p className="text-sm text-text-muted max-w-sm mt-1">Click the button above to generate a concise AI summary of this document using your local LLM.</p>
                  </div>
                )}
            ...
            {activeTab === "search" && (
              <div className="max-w-3xl flex flex-col h-full">
                <h3 className="text-xl font-serif font-semibold text-text-primary mb-4">Semantic Search</h3>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pb-4">
                  ...
                  {searchResults.map((chunk, idx) => (
                    <div key={chunk.id} className="p-4 bg-[#151514] border border-white/5 rounded-lg hover:border-white/10 transition-colors">
            ...
            {activeTab === "ocr" && (
              <div className="max-w-3xl flex flex-col h-full space-y-6">
                <div>
                  <h3 className="text-xl font-serif font-semibold text-text-primary">Optical Character Recognition (OCR) Details</h3>
                  <p className="text-sm text-text-muted mt-1">
                    This document was processed using Tesseract OCR fallback because the original PDF contains images or scanned text without embedded vector fonts.
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 bg-[#151514] border border-white/5 rounded-lg flex flex-col justify-center">
                    <span className="text-[11px] font-mono uppercase tracking-wider text-text-muted">Average OCR Confidence</span>
                    <span className="text-3xl font-bold text-accent mt-1">
                      {typeof ocrConfidence === "number" ? `${Math.round(ocrConfidence)}%` : "N/A"}
                    </span>
                  </div>
                  <div className="p-4 bg-[#151514] border border-white/5 rounded-lg flex flex-col justify-center">
                    <span className="text-[11px] font-mono uppercase tracking-wider text-text-muted">Total Pages Scanned</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-text-primary uppercase tracking-wider">Page-by-Page Confidence Matrix</h4>
                    ...
                    {ocrConfidencePerPage.length === 0 ? (
                      ...
                      <div className="max-h-[280px] overflow-y-auto divide-y divide-white/5">
                        {ocrConfidencePerPage.map((conf: number, idx: number) => {
                          ...
                          return (
                            <div key={idx} className="grid grid-cols-3 p-3 text-xs items-center text-text-secondary animate-fade-in">
