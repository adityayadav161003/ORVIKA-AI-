...
pub fn sanitize_query(raw_query: &str) -> PiiResult {
    ...
    } else if EMAIL_RE.is_match(&text) || PHONE_RE.is_match(&text) || NAME_TITLE_RE.is_match(&text) || ORG_RE.is_match(&text) {
    ...
    let common_starters = vec![
        "The", "This", "That", "They", "What", "When", "Where", "How", "Why", "Who",
        "Many", "Some", "From", "Here", "With", "Your", "Please", "Select", "Create",
        "Delete", "Update", "Search", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"
    ];
    ...
    for m in matches {
        ...
        if before_m != text { redact_count += 1; }
    }
...
mod tests {
    ...
    fn test_massive_pii_suite_10000_cases() {
        ...
        let names = vec!["Robert Chen", "Alice Vance", "Sarah Jenkins", "Devon Miller", "Elena Rostova"];
        let orgs = vec!["Acme Corp", "Google LLC", "MIT University", "Stark Industries Inc", "General Motors Co"];
        let emails = vec!["test@gmail.com", "info@org.net", "user123@yahoo.co.in", "contact@dev.io", "ceo@company.com"];
        let phones = vec!["617-555-0199", "800-555-0101", "+1 (212) 555-1234", "044 123 4567", "555.555.5555"];
        let ssns = vec!["000-12-3456", "999-88-7777", "123-45-6789", "111-22-3333", "888-00-1111"];
        let ccs = vec!["4111-2222-3333-4444", "1234 5678 1234 5678", "4532718293847281", "3782-822463-10005", "5105-1051-0510-5105"];
                        ...
                        for &s in &ssns {
                            for &c in &ccs {
                                for &t in &templates {
                                    ...
                                    assert!(!result.sanitized_text.contains(n), "Leaked name: {} in {}", n, result.sanitized_text);
                                    assert!(!result.sanitized_text.contains(o), "Leaked org: {} in {}", o, result.sanitized_text);
                                    assert!(!result.sanitized_text.contains(e), "Leaked email: {} in {}", e, result.sanitized_text);
                                    assert!(!result.sanitized_text.contains(p), "Leaked phone: {} in {}", p, result.sanitized_text);
                                    assert!(!result.sanitized_text.contains(s), "Leaked SSN: {} in {}", s, result.sanitized_text);
                                    assert!(!result.sanitized_text.contains(c), "Leaked CC: {} in {}", c, result.sanitized_text);
        ...
        assert!(count >= 10000, "Should have run at least 10,000 cases. Ran: {}", count);
    }
}
