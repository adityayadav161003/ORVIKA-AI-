...
impl SyncService {
    ...
    pub async fn perform_sync_sweep(&self) -> Result<(), String> {
        ...
        tracing::info!(
            "Performing background sync sweep to coordinator URL: {}",
            url
        );
        ...
        let mut request = client
            .post(format!("{}/push", url.trim_end_matches('/')))
            .json(&local_data);
        ...
        match request.send().await {
            Ok(response) => {
                if response.status().is_success() {
                    tracing::info!(
                        "Successfully pushed sync replication payload containing {} sessions.",
                        local_data["sessions"]
                            .as_array()
                            .map(|a| a.len())
                            .unwrap_or(0)
                    ...
                    tracing::warn!(
                        "Sync push returned non-success status code: {}",
                        response.status()
                    );
                }
            }
