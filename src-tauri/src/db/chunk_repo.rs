...
pub fn get_for_document(conn: &Connection, document_id: &str) -> AppResult<Vec<DocumentChunk>> {
    let mut stmt = conn
        .prepare("SELECT * FROM document_chunks WHERE document_id = ?1 ORDER BY chunk_index ASC")?;
...
pub fn update_embedding_ids(
    ...
    if chunk_ids.len() != embedding_ids.len() {
        return Err(crate::utils::error::AppError::Other(
            "Mismatched chunks and embeddings".into(),
        ));
    }
    ...
    let result: AppResult<()> = (|| {
        let mut stmt =
            conn.prepare("UPDATE document_chunks SET embedding_id = ?1 WHERE id = ?2")?;
...
pub fn get_chunks_by_embedding_ids(
    ...
    let params: Vec<&dyn rusqlite::ToSql> = embedding_ids
        .iter()
...
pub fn get_chunks_by_ids(conn: &Connection, ids: &[String]) -> AppResult<Vec<DocumentChunk>> {
    if ids.is_empty() {
        return Ok(vec![]);
    }
    ...
    let query = format!(
        "SELECT * FROM document_chunks WHERE id IN ({})",
        placeholders.join(",")
    );
...
pub fn update_indexing_status(
    ...
    let result: AppResult<()> = (|| {
        let mut stmt = conn.prepare(
            "UPDATE document_chunks SET weaviate_id = ?1, is_indexed = ?2 WHERE id = ?3",
        )?;

        for chunk_id in chunk_ids {
            stmt.execute(params![chunk_id, if is_indexed { 1 } else { 0 }, chunk_id])?;
        }
        Ok(())
    })();
