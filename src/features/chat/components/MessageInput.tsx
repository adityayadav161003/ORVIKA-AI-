    ...
    <div className="border-t border-border bg-white p-3 flex flex-col gap-2">
      ...
      <form onSubmit={handleSubmit} className="flex items-end gap-2">
        <textarea
          ...
          placeholder={isResearchMode ? "Ask a complex question to research..." : "Message the local model"}
          className={`max-h-40 min-h-11 flex-1 resize-none rounded-md border bg-surface px-3 py-2 text-sm leading-6 text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 transition-colors ${
            isResearchMode ? 'border-accent/50 focus:border-accent focus:ring-accent/20' : 'border-border focus:border-accent focus:ring-accent/10'
          }`}
        ...
        {sending && onCancel ? (
          ...
          <Button type="submit" disabled={!canSend} loading={sending} className={isResearchMode ? "bg-accent hover:bg-accent/90" : ""}>
            Send
          </Button>
        )}
      </form>
